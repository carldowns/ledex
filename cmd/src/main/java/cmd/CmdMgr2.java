...
@Singleton
public class CmdMgr2 implements Managed {

    private static final Logger _log = (Logger) LoggerFactory.getLogger(CmdMgr2.class);
    ...
    final ConcurrentMap<String, ICmdHandler2> _handlers = Maps.newConcurrentMap();
    ...
    @Inject
    public CmdMgr2(CmdSQL2 sql, String processID) {
    ...
    public ICmdHandler2<?> getCmdHandler (String cmdType) {
        ICmdHandler2<?> handler = _handlers.get(cmdType);
        Preconditions.checkNotNull(handler, "Handler not found for type %s", cmdType);
        return handler;
    }

    @Inject
    public void addHandler(ICmdHandler2 handler) {
    ...
    public <C extends Cmd2> C getCmd (String cmdID) {
    ...
    public void createCmd (Cmd2 cmd) {
        try {
    ...
    public void updateCmd (Cmd2 cmd) {
        try {
            String json = mapper.writeValueAsString(cmd);
    ...
    public void scheduleCmd (Cmd2 cmd, Long millisAhead) {
        try {
            String json = mapper.writeValueAsString(cmd);
    ...
    private void execCmd (CmdRec2 cmdRecord) {
        try {
            ...
            ICmdHandler2<?> handler = getCmdHandler(cmdRecord.getCmdType());
            handler.process(this, cmdRecord);
        }
        catch (Exception e) {
            _log.error("unable to execute cmd", e);
            _dao.updateCmdState(cmdRecord.getCmdID(), CmdState2.failed.name());
        }
        finally {
